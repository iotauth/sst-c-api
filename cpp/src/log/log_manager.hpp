 ...
 * @details
 * This class provides static methods to initialize the logger, set log file
 * paths, and retrieve the logger instance. It also defines a Log class for
 * creating log messages with different log levels.
 * @author Salomon Lee
 * @date 2026-03-26
 */
 ...
 * @details
 * The LogManager class provides static methods to initialize the logger, set
 * log file paths, and retrieve the logger instance. It also manages a verbosity
 * flag to control console output of log messages.
 * @note
 * This logger has taken inspiration from the logpp
 * (https://github.com/lsmon/logpp) utilizing spdlog as based logging library.
 */
class LogManager {
   public:
    ...
    /**
     * @brief Initializes the logger with a specified name, file path, and
     * verbosity setting.
     * @param logName       The name of the logger.
     * @param logFilePath   The file path where logs will be written.
     * @param verbose       A boolean flag to enable or disable console output
     * of log messages.
     * @param maxSizeBytes  Maximum size of the active log file before rotation
     * (default 50 MB).
     * @param maxFiles      Number of rotated backups to keep on disk (default
     * 10).
     * @details
     ...
     * @details
     * This method returns a reference to the logger instance. It assumes that
     * the logger has been initialized before calling this method.
     */
     ...
     * @details
     * This method returns the value of the verbosity flag, which controls
     * whether log messages are also printed to the console in addition to being
 ...
 * It captures the file name, function name, line number, and log level for each
 * message.
 */
class Log {
   public:
    /**
     * @brief Constructs a log message with the specified file name, function
     * name, line number, and log level.
     * @param fileName The name of the file where the log message is generated.
     * @param funcName The name of the function where the log message is
     * generated.
     * @param line The line number where the log message is generated.
     * @param l The log level for the message.
     * @details
     * This constructor initializes the log message with contextual information
     * such as the file name, function name
     */
    Log([[maybe_unused]] const std::string& fileName,
    ...
    /**
     * @brief Overloads the stream insertion operator to construct the log
     * message.
     ...
     * @details
     * This operator allows for a stream-like syntax when constructing log
     * messages. It converts the value to a string and appends it to the log
     * message stream.
     */
    template <class T>
...
#define LOG_INF                                        \
    Log(getFileName(__FILE__), __FUNCTION__, __LINE__, \
