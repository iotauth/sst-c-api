  ...
  while (std::getline(file, line)) {
    ...
    if (key == "name") {
      ...
      snprintf(cfg.auth_ip_addr, sizeof(cfg.auth_ip_addr), "%.*s",
               static_cast<int>(sizeof(cfg.auth_ip_addr) - 1), value.c_str());
    } else if (key == "auth_port_num") {
      cfg.auth_port_num = std::stoi(value);
    } else if (key == "entity_server_ip_addr") {
      snprintf(cfg.entity_server_ip_addr, sizeof(cfg.entity_server_ip_addr),
               "%.*s", static_cast<int>(sizeof(cfg.entity_server_ip_addr) - 1),
               value.c_str());
      ...
      if (bracket_start != std::string::npos &&
        ...
        int idx = std::stoi(
            key.substr(bracket_start + 1, bracket_end - bracket_start - 1));
        if (idx >= 0 && idx < 2) {
...
std::optional<SessionKey> SST_API::get_session_key_by_id(
  ...
  for (int i = 0; i < session_key_list_->num_key; ++i) {
    ...
    if (std::memcmp(session_key_id.data(), session_key_list_->s_key[idx].key_id,
...
void SST_API::perform_auth_handshake(const std::string& purpose) {
  ...
  try {
    ...
    // Parse AUTH_HELLO payload: auth_id (4 bytes, big-endian) + auth_nonce (8
    // bytes)
    ...
    // Variable-length integer encoding: each byte has high bit set except the
    // last.
    ...
    ret = Crypto::public_encrypt(plaintext_payload, plaintext_len,
                                 RSA_PKCS1_OAEP_PADDING, ctx_->pub_key.get(),
    ...
    ret =
        Crypto::sha256_sign(encrypted, static_cast<unsigned int>(encrypted_len),
                            ctx_->priv_key.get(), signature, &sig_len);
    if (ret != 0) {
      throw SST_Exception("Failed to sign session key request");
    }

    // --- 2d. Build the IoTSP message: [msgType][payloadLen varint][ciphertext
    // + signature] ---
    ...
    for (int i = 0; i < num_keys_resp && i < static_cast<int>(MAX_SESSION_KEY);
         ++i) {
...
int SST_API::recv_from_auth(int sock, unsigned char* buf, size_t len) {
  ...
  // This avoids blocking when the server sends a short message (e.g. AUTH_HELLO
  // = 14 bytes) into a buffer sized for MAX_SECURE_COMM_MSG_LENGTH. Note:
  // read_iotsp_header() already consumed msgType (1 byte) + varint payloadLen
  ...
  while (total < payload_len) {
    ...
    if (n <= 0) {
      LOG_ERR << "Failed to receive payload from Auth server (got " << total
...
std::unique_ptr<SST_Session> SST_Session::connect_to_server(
  ...
  int ret =
      ssl_socket.InitClientContext(nullptr,  // cert_file (optional for client)
...
int SST_Session::send_message(const std::vector<uint8_t>& data) {
  ...
  int ret = Crypto::symmetric_encrypt_authenticate(
      ...
      AES_IV_SIZE, session_ctx_->s_key.enc_mode, session_ctx_->s_key.hmac_mode,
...
std::vector<uint8_t> SST_Session::read_message() {
  ...
  if (msg_len > MAX_SECURE_COMM_MSG_LENGTH) {
    throw SST_Exception(
        "Received message too large: " + std::to_string(msg_len) +
        " bytes (max: " + std::to_string(MAX_SECURE_COMM_MSG_LENGTH) + ")");
  }
