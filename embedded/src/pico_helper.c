...
const uint8_t* keyram_get(void){
    return g_valid ? g_session_key : NULL;
}
...
typedef struct {
    ...
    uint8_t  reserved[FLASH_PAGE_SIZE
                      - 4 - 4 - 4
                      - SST_KEY_SIZE
                      - 32];
} key_flash_block_t; /* exactly 256 bytes */

static void compute_key_hash(const uint8_t *data, size_t len, uint8_t *out_hash) {
...
static bool read_key_from_sector(uint32_t sector_offset, uint8_t *out_key, uint32_t *out_counter) {
...
static bool program_sector_page0(uint32_t sector_offset, const key_flash_block_t *blk) {
    ...
    return validate_flash_block(rd)
        ...
        && rd->counter == blk->counter
        && rd->version == blk->version
        && rd->magic == blk->magic;
}
...
bool load_session_key(uint8_t *out) {
    ...
    if (a_ok && b_ok) {
        if (b_cnt >= a_cnt) { memcpy(out, b_key, SST_KEY_SIZE); return true; }
        else                { memcpy(out, a_key, SST_KEY_SIZE); return true; }
    } else if (a_ok) {
        memcpy(out, a_key, SST_KEY_SIZE); return true;
    } else if (b_ok) {
        memcpy(out, b_key, SST_KEY_SIZE); return true;
    }
    return false;
}
...
bool is_key_zeroed(const uint8_t *key) {
    for (size_t i = 0; i < SST_KEY_SIZE; i++) if (key[i] != 0) return false;
    return true;
}

void get_random_bytes(uint8_t* buffer, size_t len) {
    for (size_t i = 0; i < len; i++) buffer[i] = (uint8_t)(get_rand_32() & 0xFF);
}
