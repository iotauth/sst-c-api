...
"""
latency_plot.py
Plot Latency (avg_us) vs Number of Malicious Clients where
each CSV file contributes exactly one point.

- Converts latency from microseconds to milliseconds on the y-axis.
- Disables scientific notation on the y-axis.
...
REQUIRED_COLS = ["malicious_number", "avg_us"]
...
def point_from_file(csv_path: str):
    ...
    y_ms = float(df_clean["avg_us"].mean()) / 1000.0
    return {"source": csv_path, "malicious_number": x, "latency_ms": y_ms}
...
def main():
    ap = argparse.ArgumentParser(description="Plot Latency vs Number of Malicious Clients")
    ...
    ap.add_argument("--out", default="latency.pdf", help="Output filename (PDF). Default: latency.pdf")
    ...
    ap.add_argument("--ylim", nargs=2, type=float, metavar=("YMIN","YMAX"), help="Clamp Y axis to [YMIN, YMAX] (ms)")
    args = ap.parse_args()
    ...
    plt.plot(df["malicious_number"], df["latency_ms"], marker="o")
    ax = plt.gca()
    ...
    # Disable scientific notation on y and force integer ticks
