...
// Send TCP SYN packets to (dst_ip, dst_port) with src IP spoofed.
...
// @return 0 on success, nonzero on error.
bool send_udp_packets(const char* src_ip_str,
                                     const char* dst_ip,
...
// @return 0 on success, nonzero on error.
int get_src_ip(const char* dst_ip, unsigned short dst_port, char* out_ip,
               size_t out_ip_len);
