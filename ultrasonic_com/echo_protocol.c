...
#define SOFTWARE_OVERHEAD_SEC \
    0.0  // processing time for the prove (calculate avarage by testing)
#define TOLERANCE_MARGIN_M \
    00.0  // Margin in meters to account for timing jitter
...
void run_prover(const char* mic_device, const char* spk_device) {
    ...
    printf(
        "PROVER: Listening for Challenge from Verifier (Ctrl+C to stop)...\n");
    ...
    while (1) {
        ...
        int decoded_bytes =
            ggwave_decode(instance, buffer, frames_read * 2, rx_payload);
        if (decoded_bytes > 0) {
            ...
            if (rx_payload[0] == 'N' && decoded_bytes == 5) {
                ...
                // Flush the microphone buffer so we don't decode our own
                // transmission
                snd_pcm_drop(mic);
                snd_pcm_prepare(mic);
...
void run_verifier(const char* mic_device, const char* spk_device,
                  uint16_t permitted_dist_cm) {
    ggwave_Parameters parameters = ggwave_getDefaultParameters();
    parameters.sampleRate = SAMPLE_RATE;
    parameters.sampleFormatInp = GGWAVE_SAMPLE_FORMAT_I16;
    parameters.sampleFormatOut = GGWAVE_SAMPLE_FORMAT_I16;
    ...
    while (1) {
        ...
        if (elapsed > 1.0) {
            printf(
                "VERIFIER: Timeout waiting for echo. Broadcasting new "
                "Nonce...\n");
            generate_nonce(
                expected_nonce);  // generate new nonce for next bradcast
            strcpy(msg_nonce + 1, expected_nonce);
        ...
        if (decoded_bytes > 0) {
            ...
            if (rx_payload[0] == 'N' && decoded_bytes == 5) {
                ...
                if (strncmp(rx_payload + 1, expected_nonce, 4) == 0) {
                    ...
                    printf(
                        "VERIFIER: Valid Echo Received! RTT(actual): %f "
                        "seconds\n",
                        rtt);
                    ...
                    if (measured_dist_m <=
                        permitted_dist_m + TOLERANCE_MARGIN_M) {
                        printf(
                            "VERIFIER: ACCEPTED! Measured Dist: %.2fm <= "
                            "Permitted Dist: %.2fm (Margin: %.1fm)\n",
                            measured_dist_m, permitted_dist_m,
                            TOLERANCE_MARGIN_M);
                    } else {
                        printf(
                            "VERIFIER: REJECTED! Measured Dist: %.2fm exceeds "
                            "Permitted Dist: %.2fm\n",
                            measured_dist_m, permitted_dist_m);
                    }
...
int main(int argc, char** argv) {
    ...
    if (strcmp(mode, "prover") == 0) {
        if (argc < 4) {
            fprintf(stderr,
                    "Usage: %s prover <capture_device> <playback_device>\n",
                    argv[0]);
            return 1;
        }
        ...
        if (argc < 5) {
            fprintf(stderr,
                    "Usage: %s verifier <capture_device> <playback_device> "
                    "<permitted_dist_cm>\n",
                    argv[0]);
            return 1;
        }
