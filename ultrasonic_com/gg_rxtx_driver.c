...
void run_tx(const char* device, const char* payload) {
    ...
    setup_ggwave_freq();  // run this befor initilizing the ggwave, for custome
                          // configuration
...
void run_rx(const char* device) {
    ...
    // 3. Listen Loop
    printf(
        "Listening for ultrasound on 10.5 kHz - 15 kHz band on device %s...\n",
        device);
    ...
    while (1) {
        snd_pcm_sframes_t frames_read =
            snd_pcm_readi(pcm_handle, buffer, frames_to_read);
